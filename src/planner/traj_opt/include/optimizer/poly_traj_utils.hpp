...
namespace poly_traj
{
    ...
    typedef Eigen::Matrix<double, dim, 6> CoefficientMat;
    typedef Eigen::Matrix<double, dim, 5> VelCoefficientMat;
    typedef Eigen::Matrix<double, dim, 4> AccCoefficientMat;
        ...
        inline double getMaxVelRate() const
        {
            ...
            else
            {
                     ...
                     it++)
                {
                    if (0.0 <= *it && 1.0 >= *it)
                    {
                        tempNormSqr = getVel((*it) * duration).topRows(3).squaredNorm(); //the first three variable for load 
    ...
    private:
        ...
        inline void addPropCtoT(const Eigen::MatrixXd &adjGdC, EIGENVEC &gdT) const
        {
            ...
            Eigen::RowVectorXd negVel(dim), negAcc(dim), negJer(dim), negSnp(dim), negCrk(dim);
                                      ...
                                      Eigen::MatrixXd &gdC) const
        {
            ...
            Eigen::Matrix<double, dim, 1> beta0, beta1, beta2, beta3;
            ...
            Eigen::Matrix<double, 6, dim> gradViolaVc, gradViolaAc;
            ...
            for (int i = 0; i < N; i++)
            {
                const auto &c = b.block<6, dim>(i * 6, 0);
                step = T1(i) / cons(i);
                s1 = 0.0;
                innerLoop = cons(i) + 1;
                for (int j = 0; j < innerLoop; j++)
                {
                    ...
                    for (int k = 0; k < K; k++)
                    {
                        ...
                        if (violaPos > 0.0)
                        {
                            ...
                            violaPosPenaD *= 3.0;
                            gdC.block<6, dim>(i * 6, 0) += omg * step * ci(0) * violaPosPenaD * beta0 * outerNormal.transpose();
                    ...
                    if (violaVel > 0.0)
                    {
                        ...
                        gdC.block<6, dim>(i * 6, 0) += omg * step * ci(1) * violaVelPenaD * gradViolaVc;
                    ...
                    if (violaAcc > 0.0)
                    {
                        ...
                        gdC.block<6, dim>(i * 6, 0) += omg * step * ci(2) * violaAccPenaD * gradViolaAc;
